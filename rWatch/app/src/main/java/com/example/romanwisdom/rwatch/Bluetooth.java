...
public class Bluetooth {
    ...
    public void sendMessage(String message) {
        ...
        if (message.length() > 0) {
            //byte[] transEnd = {(byte)0xFD};
            //byte[] transBegin = {(byte)0xFC};

            byte[] messageBytes = message.getBytes();
            /*byte[] send = new byte[message.getBytes().length + transEnd.length + transBegin.length];

            System.arraycopy(transBegin, 0, send, 0, transBegin.length);
            System.arraycopy(messageBytes, 0, send, transBegin.length, messageBytes.length);
            System.arraycopy(transEnd, 0, send, send.length-1, transEnd.length);*/

            this.write(messageBytes);
        }
    }

    public void sendTransaction(int transactionType, String message){
        if (this.getState() != Bluetooth.STATE_CONNECTED) {
            Log.w(TAG, "bluetooth is not connected");
            return;
        }
        if (message.length() > 0) {
            this.write((message = "{" + transactionType + message + "}").getBytes());
        }
